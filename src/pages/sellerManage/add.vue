        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="公司名称" prop="name">
              <el-input placeholder="请输入公司名称" v-model.trim="sellerForm.name"></el-input>
            </el-form-item>
          ...
          <el-col :span="12">
            <el-form-item label="纳税人识别号" prop="taxCode">
              <el-input placeholder="请输入纳税人识别号" v-model.trim="sellerForm.taxCode"></el-input>
            </el-form-item>
  ...
  export default {
    data() {
      const validatorCompanyName = (rule, value, callback) => {
        if (validate.validateCompanyName(value)) {
          callback();
        } else {
          callback(new Error('公司名称格式不对'));
        }
      };
      ...
      const validatorTaxCode = (rule, value, callback) => {
        if (validate.validateTaxCode(value)) {
          callback();
        } else {
      ...
      return {
        ...
        rules: {
          ...
          taxCode: [
            ...
            {validator: validatorTaxCode, trigger: 'blur'}
          ],
          phone: [
    ...
    methods: {
      ...
      handleSave(sellerForm) {
        this.$refs[sellerForm].validate((valid) => {
          if (valid) {
            this.loading = true;
            fetch.post('/company/save', util.stringify(this.sellerForm)).then(({msg}) => {
              this.loading = false;
              this.$message({
                type: 'success',
                message: msg
