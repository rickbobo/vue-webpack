...
<script>
  ...
  export default {
    ...
    created() {
      this.roleForm.id = this.$route.params.state;
    },
    methods: {
      ...
      handleSave(roleForm) {
        this.$refs[roleForm].validate((valid) => {
          if (valid) {
            this.loading = true;
            fetch.post('/role/update', util.stringify(this.roleForm)).then(({msg}) => {
              this.loading = false;
              this.$message({
                type: 'success',
                message: msg
    ...
    mounted() {
      fetch.get('/role/get/' + this.roleForm.id).then(({data}) => {
        util.copyProperties(this.roleForm, data);
      }).catch(() => {
      });
    }
  }
